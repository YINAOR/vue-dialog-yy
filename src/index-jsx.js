...
export default {
	install: (Vue, { store, router, registerComponent }) => {
		...
		if (Vue.options.components) {
			if (!Vue.options.components.ElButton) {
				ElementUIComponent.ElButton = Button
			}
		}
		...
		instance = new Vue({
			...
			render() {
				const cancelButton = showCancelButton ? (
					<el-button plain size={cancelButtonSize} on-click={() => this.handleClose('cancel')}>
						{cancelButtonText}
					</el-button>
				) : (
					''
				...
				const confirmButton = showConfirmButton ? (
					<el-button plain size={confirmButtonSize} type="primary" on-click={() => this.handleClose('confirm')}>
						{confirmButtonText}
					</el-button>
				) : (
					''
				...
				const customButtonsDom = customButtons.map(btn => {
					...
					return (
						<el-button type={btn.type || ''} size={btn.size || 'small'} on-click={clickEvent} loading={btn.loading}>
							{btn.text}
						</el-button>
					)
				})
				...
				return (
					<Dialog
						...
						on-on-closed={() => this.handleClose('close')}
						on-on-minimized={this.handleMinimized}
						on-on-maximized={this.handleMaximized}
						on-on-start-drag={this.handleStartDrag}
						on-on-stop-drag={this.handleStopDrag}
						on-on-drag={this.handleDrag}
						on-on-start-resize={this.handleStartResize}
						on-on-stop-resize={this.handleStopResize}
						on-on-resize={this.handleResize}
						on-on-click={this.handleClick}
					>
						<ChildComponent
							ref="child"
							props={separate(cOpts, _this).props}
							on-on-closed={() => this.handleClose('close')}
							on={separate(cOpts, _this, this.handleClose).events}
						/>
						{footer}
					</Dialog>
				)
