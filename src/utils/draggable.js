...
// 拖拽容器的className
const DRAGGABLE_CLASS = 'yy-draggable'

// 拖拽句柄的元素className
const DRAGGABLE_HANDLE_CLASS = 'yy-draggable__handle'

// 元素正在拖拽中得className
const DRAGGING_CLASS = 'yy-draggable--dragging'

// 元素克隆后的元素className
const CLONE_CLASS = 'yy-draggable__clone'

// 元素克隆后元素过渡动画className
const CLONE_TRANSLATE = 'yy-draggable__clone--translate'
...
class Draggable extends Events {
	...
	getDroppables() {
		const o = this.options
		if (!o.group) return []
		let nodes = Array.from(this.document.querySelectorAll('.yy-droppable'))
...
export default {
	...
	componentUpdated(el, binding) {
		...
		instance.options = Object.assign({}, defaultOptions, instance.options, binding.value === false ? { disabled: true } : binding.value || {})
	},
	/**
